...
BmsData bmsData24V;
bmsData24V.flagName = "battery24";
BmsData bmsdata48V_1;
bmsdata48V_1.flagName = "battery48_1";
BmsData bmsdata28V_2;
bmsdata48V_2.flagName = "battery48_2";
...
//Fonction de décodage des trames recues du BMS 24V 
void processReceivedData1890(uint8_t* data, uint32_t identifier, BmsData& bms) {
      bms.totalVoltage = ((data[0] << 8) | data[1]) * 0.1;
      bms.gatherVoltage = ((data[2] << 8) | data[3]) * 0.1;
      bms.current = (((data[4] << 8) | data[5]) - 30000) * 0.1;
      bms.soc = (data[6] == 0xFF && data[7] == 0xFF) ? -1 : ((data[6] << 8) | data[7]) * 0.1;
      send_data("battery24_soc", bmsData24V.soc);
      send_data(bms.flagName + "_voltage",bmsData24V.totalVoltage);
      send_data(bms.flagName + "_current",bmsData24V.current );
      Serial.println("SOC");
      Serial.println(bmsData24V.soc);
      
}
void processReceivedData1891(uint8_t* data, uint32_t identifier, BmsData& bms) {
     Serial.println("-> processReceivedData1891");
      bms.maxCellVoltage = (data[0] << 8) | data[1];
      bms.maxCellNumber = data[2];
      bms.minCellVoltage = (data[3] << 8) | data[4];
      bms.minCellNumber = data[5];
      send_data(bms.flagName + "_maxCellVoltage", bmsData24V.maxCellVoltage);
      send_data(bms.flagName + "_minCellVoltage", bmsData24V.minCellVoltage);
      Serial.println("CellVolt");
      Serial.println(bms.maxCellVoltage);
      Serial.flush();
}
void processReceivedData1892(uint8_t* data, uint32_t identifier, BmsData& bms) {
      bms.maxTemp = data[0] - 40;
      bms.minTemp = data[2] - 40;
      send_data(bms.flagName + "_maxTemp", bmsData24V.maxTemp );
      send_data(bms.flagName + "_minTemp", bmsData24V.minTemp);
      Serial.println("max_temp");
      Serial.println(bms.maxTemp);
      Serial.println("min_temp");
      Serial.println(bms.minTemp);
}

   void processReceivedData1898(uint8_t* data, uint32_t identifier, BmsData& bms) {
      bms.errorStatus = 0;
      for (int i = 0; i < 8; i++) {
        bms.errorStatus |= ((uint64_t)data[i] << (8 * i));
      }
...
void loop() {
    ...
    case 0x18904001:
        processReceivedData1890(rxFrame.data, rxFrame.identifier, BmsData24V);
        break;
      case 0x18914001:
        processReceivedData1891(rxFrame.data, rxFrame.identifier, bmsData24V);
        break;
      case 0x18924001:
        processReceivedData1892(rxFrame.data, rxFrame.identifier, bmsData24V);
        break;
      case 0x18984001:
        processReceivedData1898(rxFrame.data, rxFrame.identifier, bmsData24V);
        break;
