...
void processSystemStateFrame(CanFrame &frame) {
    if(frame.data_length_code >= 4) {
        ...
        send_data("pac_system_state", fuelCellData.systemStateStr);
...
void processH2SensorsFrame(CanFrame &frame) {
    if(frame.data_length_code >= 8) {
        ...
        send_data("h2_pac_status", h2SensorsData.pacSensorOk ? "OK" : "ERREUR");
        send_data("h2_bottle_status", h2SensorsData.bottlesSensorOk ? "OK" : "ERREUR");
    }
}
...
void setup() {
    ...
    // Configurer la broche en sortie Transistor
    pinMode(transistorPinCapteurH2, OUTPUT);
